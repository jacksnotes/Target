...
const bundleId = "space.manus.study.planner.t20260319100047";
...
export function getApiBaseUrl(): string {
  ...
  // On native, use the experience URL (machine IP)
  if (ReactNative.Platform.OS !== "web") {
    // try Constants.expoConfig.hostUri (SDK 47+) or Constants.manifest.debuggerHost
    const hostUri = Constants.expoConfig?.hostUri || (Constants as any).manifest?.debuggerHost;
    if (hostUri) {
      // hostUri is usually "IP:PORT"

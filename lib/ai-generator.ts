...
export async function generateTaskPayload<T extends ExecutionType>(
  ...
  switch (task.executionType) {
    case "flashcard":
      ...
      promptDetails = `生成 ${options.isAdvanced ? "20-30" : "10-15"} 个核心词汇或知识点的翻转卡片。适合背诵和动态测验。`;
      break;
    case "reading":
      schema = `{ "title": "文章标题", "content": "文章正文内容", "wordCount": 1000 }`;
      promptDetails = `生成一篇高质量、专业且引引人深思的学习内容/文章。
注意：
1. 勿使用 Markdown 标题符号（如 #）或强调符号（如 *）。
2. 使用纯文本，段落之间用两个换行符分隔。
3. 标题直接单独成行（不需要特殊符号），保持排版清爽易读。
字数适中（${options.isAdvanced ? "1500-2500" : "800-1500"}字）。`;
      break;
    case "quiz":
      schema = `{ "questions": [{ "question": "问题描述", "options": ["选项A", "选项B", "选项C", "选项D"], "answerIndex": 0 }] }`;
      promptDetails = `生成 ${options.isAdvanced ? "20" : "12"} 个单项选择题，覆盖该模块的核心知识、易错点和应用场景，难度逐步提高。${options.isAdvanced ? "题目应包含深度逻辑分析和多知识点综合。" : ""}options 是一个字符串数组，answerIndex 是正确选项在数组中的索引(0-3)。题目必须足够区分理解和猜测，不要出送分题。`;
      break;
    case "coding":
